...
def_learning_rate = 1e-5
def_steps = 30
def_guidance_scale = 9

learning_rates = [1e-4, 5e-4, 1e-6, 5e-6]
stepss = [10, 15, 45, 60]
guidances = [2, 7, 10 , 13]

cur_index = cur_task%4

if (cur_task//4)==0:
    learning_rate = learning_rates[cur_index]
    steps = def_steps
    scale = def_guidance_scale
elif (cur_task//4)==1:
    learning_rate = def_learning_rate
    steps = stepss[cur_index]
    scale = def_guidance_scale
else:
    learning_rate=def_learning_rate
    steps = def_steps
    scale = guidances[cur_index]
...
fixed_cb = FixedEval(fixed_batch, every=300, outdir="eval_fixed", steps=steps, eta=0.0, scale=scale)
